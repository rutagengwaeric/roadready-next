"use strict";

export const signsKeywords = [
  {
    id: 11,
    title: "Iki cyapa cyerekana iki?",
    img: "/assets/images/urusisiro.png",
    options: [
      "urusisiro.",
      "Aho banyura bazengurutse.",
      "A na B ni ibisubizo by’ukuri",
      "Nta gisubizo cy’ukuri kirimo",
    ],
    correctAnswer: 0,
  },
  {
    id: 12,
    title: "Iki cyapa cyerekana iki?",
    img: "/assets/images/makuzungu.jpg",
    options: [
      "ikinyabizigo cy'omurwego rwa B",
      "makuzungu",
      "ikinyabizigo cy'o murwego rwa D",
      "Ibisubizo byose nibyo",
    ],
    correctAnswer: 1,
  },

  {
    id: 13,
    title: "Iki cyapa cyerekana iki?",
    img: "/assets/images/isangano.jpg",
    options: [
      "Aho banyura bazengurutse",
      "isangano",
      "A na B ni ibisubizo by’ukuri",
      "Nta gisubizo cy’ukuri kirimo",
    ],
    correctAnswer: 1,
  },

  {
    id: 14,
    title: "Iki cyapa cyerekana iki?",
    img: "/assets/images/rumomoroki.jpg",
    options: [
      "makuzungu",
      "Rumoruki",
      "A na B nibisubizo byukuri",
      "Ntagisubizo cy'ukuri kirimo",
    ],
    correctAnswer: 1,
  },

  {
    id: 15,
    title: "Iki kinyabiziga ni ..",
    img: "/assets/images/ikinyamitende itatu.jpg",
    options: [
      "ipikipiki.",
      "motor yamapine atatu",
      "ikinyamitende itatu",
      "Ntagisubizo cy'ukuri kirimo.",
    ],
    correctAnswer: 2,
  },

  {
    id: 16,
    title: "Iki kinyabiziga ni ..",
    img: "/assets/images/ikinyamitende ine.jpg",
    options: [
      "ikinyamitende ine",
      "ipikipiki.",
      "A na B nibisubizo by'ukuri",
      "Ntagisubizo cy'ukuri kirimo.",
    ],
    correctAnswer: 0,
  },

  {
    id: 17,
    title: "Iki kinyabiziga ni ..",
    img: "/assets/images/Ipikipiki.jpg",
    options: [
      "velomoteri",
      "ipikipiki.",
      "motor yamapine atatu",
      "Ntagisubizo cy'ukuri kirimo.",
    ],
    correctAnswer: 1,
  },

  {
    id: 18,
    title:
      "Ugeze bwa mbere ahabereye impanuka yo mu muhanda harimo inkomere wakora iki ? ",
    img: "../image/288.png",
    options: [
      "gusohora inkomere mu kinyabiziga",
      "kubaha icyo kunywa",
      "ku menyesha impanuka no guhamagara ubutabazi",
      "nta gisubizo cy’ukuri kirimo",
    ],
    correctAnswer: 2,
  },

  {
    id: 19,
    title: "Muri ibi binyabiziga n’ikihe gihagaze nabi?",
    img: "../image/310.png",
    options: [
      "Ibinyabiziga byombi",
      "Ikinyabiziga cy’icyatsi",
      "Ikinyabiziga cy’umutuku",
      "Nta n’imwe",
    ],
    correctAnswer: 0,
  },

  {
    id: 20,
    title:
      "Mbere yuko umuyobozi w’ikinyabiziga akata ibumoso mu  nzira nyabagendwa,  nihe ikinyabiziga kigomba kuba kiri ?",
    img: "../image/340.png",
    options: [
      "Mu ruhande rw’iburyo bw’inzira nyabagendwa",
      "Gusa iburyo bwo hagati y’inzira nyabagendwa",
      "Muruhande  urwarirwo rwo  hagati mu nzira nyabagendwa",
      "Mu ruhande rw’ibumoso bw’inzira nyabagendwa",
    ],
    correctAnswer: 1,
  },
];

export const Test1 = [
  {
    id: 430,
    title:
      "Niki wakora mugihe usanze mu bimenyetso bimurika harimo ibara ry’umuhondo:",
    img: "../image/215.png",
    options: [
      "Kongera umuvuduko",
      "Kugumana umuvuduko wari uriho",
      "Kwitegura guhagarara",
      "Gufata feri cyane",
    ],
    correctAnswer: 2,
  },

  {
    id: 431,
    title: "Iki cyapa cyivuga iki?",
    img: "../image/229.png",
    options: [
      "Umuvuduko ntarengwa 30 km/h",
      "Iherezo ry’umuvuduko  muke ntarengwa utegetswe",
      "Iherezo ry’Umuvuduko muto utegetswe",
      "Umuvuduko uri hejuru 30 km/h",
    ],
    correctAnswer: 1,
  },

  {
    id: 283,
    title: "Icyapa gikurikira  kivuze iki?",
    img: "../image/230.png",
    options: [
      "Ntihanyurwa",
      "Birabujijwe guhagarara umwanya munini",
      "Umuvuduko utarengeje",
      "Inzira yabanyeshuli",
    ],
    correctAnswer: 1,
  },
  {
    id: 284,
    title:
      "Inzira nyabagendwa ifite ibyerekezo  bibiri, uruhande rw’ibumoso rudufasha iki?",
    img: "../image/231.png",
    options: [
      "Kunyuranaho gusa,  ntugaruke  iburyo bwawe",
      "Kunyuranaho cyangwa ugakatira ibumoso",
      "Hemerewe kugenda imodoka zihuta gusa",
      "Gukatira iburyo gusa utanyuranyeho",
    ],
    correctAnswer: 1,
  },

  {
    id: 420,
    title: "Iki cyapa gisobanura iki?",
    img: "../image/242.png",
    options: [
      "Iherezo ry’ umuhanda urombeje w’ibice byinshi ",
      "Birabujijwe kunyura mu mu muhanda w’ ikindi cyerekezo",
      "Birabujijwe kunyuranaho",
      "Birabujijwe guhagara ku iteme",
    ],
    correctAnswer: 0,
  },

  {
    id: 285,
    title: "Iki cyapa gisobanura iki?",
    img: "../image/234.png",
    options: [
      "Iherezo ry’ibibuzwa byose mu karere  ku binyabiziga bigenda",
      "Ntihemerewe kuhahagarara",
      "Umuvuduko ntarengwa wemewe",
      "Nta gisubizo cy’ukuri kirimo",
    ],
    correctAnswer: 0,
  },

  {
    id: 286,
    title: "iki cyapa gisobanura iki?",
    img: "../image/276.png",
    options: [
      "Uburenganzira bwo gutambuka mbere",
      "Nta kinyabiziga kigendeshwa na  moteri",
      "ibyerekezo bibiri by’umuhanda",
      "Birabujijwe kunyuranaho",
    ],
    correctAnswer: 3,
  },

  {
    id: 429,
    title:
      "Niki ugomba gukora igihe wegereye ikimenyetso kimurika kiva mucyatsi kijya mumuhondo?",
    img: "../image/313.png",
    options: [
      "Ongera umuvuduko kugirango usoze ikoni",
      "Komeza kuko itara ry’icyatsi rigiye kwaka",
      "Hagarara niba utateza ibyago",
      "Komeza ubwitonzi witegura guhagarara mugihe itara rihindutse umutuku",
    ],
    correctAnswer: 2,
  },

  {
    id: 287,
    title: "Imbere yawe  iki cyapa kikubwiye iki?",
    img: "../image/238.png",
    options: [
      "Umuvuduko ntarengwa wemewe",
      "Iherezo ry’ibyo wabuzwaga",
      "Guhagarara umwanya munini n’umwanya moto ntibyemewe",
      "Birabujijwe kuhinjira",
    ],
    correctAnswer: 2,
  },

  {
    id: 288,
    title: "Iki cyapa gisobanura iki?",
    img: "../image/239.png",
    options: [
      "Umuhanda uzenguruka",
      "Igice cy’umuhanda uzenguruka",
      "Aho banyura bazengurutse",
      "Ibisubizo byose nibyo",
    ],
    correctAnswer: 2,
  },

  {
    id: 289,
    title: "Iki cyapa gisubanura iki?",
    img: "../image/240.png",
    options: [
      "Iteme ridahoraho",
      "Umuhanda utaringaniye",
      "Umuhanda w’injira mu kuzimu",
      "Ubutaka bworoshye",
    ],
    correctAnswer: 1,
  },

  {
    id: 416,
    title:
      "Muri ibi byapa bikurikira ni ikihe cyerekana ko umuyobozi ukibonye yemerewe gutambuka mbere y'abaturutse aho agana mu nzira ifunganye:",
    img: "../image/424.png",
    options: ["Icyapa B6", "Icyapa A19", "Icyapa B3", "Icyapa A22a"],
    correctAnswer: 0,
  },

  {
    id: 413,
    title: "Iki cyapa cyerekana:",
    img: "../image/373.png",
    options: [
      "ifungana ry’umuhanda",
      "ifungana ry’umuhanda n’akayira gasatira umuhanda i bumoso",
      "umuhanda utaringaniye",
      "nta gisubizo cy’ukuri kirimo",
    ],
    correctAnswer: 3,
  },

  {
    id: 418,
    title: "Iki cyapa gisobanura iki ?",
    img: "../image/412.png",
    options: [
      "Birabujijwe  ku binyabiziga bitwara abakozi ba leta",
      "Birabujijwe guhagara umwanya munini",
      "Birabujijwe ku binyabiziga by’abikorera ki giti cyabo",
      "Parikingi",
    ],
    correctAnswer: 1,
  },

  {
    id: 290,
    title: "Iki cyapa gisobanura iki?",
    img: "../image/241.png",
    options: [
      "Umuyaga w’intambike",
      "Urusaku rwo mu muhanda",
      "Ikibuga cy’indege",
      "Ibisubizko byose nibyo",
    ],
    correctAnswer: 0,
  },

  {
    id: 291,
    title: "Iki Cyapa Gisobanura Iki?",
    img: "../image/242.png",
    options: [
      "Iherezo Ry’inzira Y’abanyamaguru",
      "Iherezo Ry’umuhanda Urombereje W’ibice Byinshi",
      "A Na B Ni Ibisubizo By’ukuri",
      "Nta nzira ihari",
    ],
    correctAnswer: 1,
  },

  {
    id: 422,
    title:
      "Mu gihe  utwaye ikinyabiziga uva kuri A ugana kuri B,  Iki kimenyetso kiri mu muhanda kivuze iki ?",
    img: "../image/433.png",
    options: [
      "Umuyobozi w’ikinyabiziga ashobora kunyuranaho arenze umurongo wera udacagaguye",
      "Umuyobozi w’ikinyabiziga abujijwe kunyuranaho arenze imirongo yera",
      "Umuyobozi w’ikinyabiziga yemerewe kunyuranaho",
      "Abayobozi b’ibinyamitende gusa bemerewe kunyuranaho barenze umurongo wera udacagaguye",
    ],
    correctAnswer: 1,
  },

  {
    id: 424,
    title: "Ibi byapa byo mu muhanda birambuza kunyuranaho ibumoso ?",
    img: "../image/388.png",
    options: [
      "yego",
      "yego, iyo ufite umuvuduo wa 90km/h",
      "oya",
      "ntagisubizo cy’ukuri",
    ],
    correctAnswer: 0,
  },

  {
    id: 292,
    title: "Iki cyapa gisobanura Iki?",
    img: "../image/243.png",
    options: [
      "Iherezo Ry’umuhanda Wi Byerekezo Bibiri",
      "Iteme rinini Kandi rirerire",
      "Ifungana Ry’umuhanda",
      "Iherezo ry’iteme  rifunganye",
    ],
    correctAnswer: 2,
  },

  {
    id: 415,
    title: "Iki cyapa gisobanura iki?",
    img: "../image/410.png",
    options: [
      "Birabujijwe guhindukira",
      "Birabijijwe gusubira inyuma",
      "Umuhanda unyerera imbere",
      "Ntibyemewe kugendera mu byerekezo byombi",
    ],
    correctAnswer: 0,
  },

  {
    id: 293,
    title: "Iki cyapa gisobanura iki?",
    img: "../image/244.png",
    options: [
      "Isangano rifite ishusho  ya T",
      "Inzira idakomeza",
      "Aho baterefonera",
      "Nta gisubizo cy’ukuri",
    ],
    correctAnswer: 1,
  },

  {
    id: 294,
    title: "Iki cyapa gisobanura iki?",
    img: "../image/245.png",
    options: [
      "Inzira y’abanyeshuri",
      "Abanyamaguru ntibemerewe",
      "Agace k’abanyamaguru nta  kinyabiziga",
      "Hegereye aho abanyamaguru bambukira",
    ],
    correctAnswer: 3,
  },

  {
    id: 408,
    title: "Umuhanda wambukiranya inzira ya gariyamoshi",
    img: "../image/393.png",
    options: [
      "ntabwo nakomeza urugendo rwanjye. Ngomba gihita mpagarara",
      "nshobora gukomeza nkambuka umuhanda kubera ko uruzitiro rufunguye",
      "ntagisubizo cy’ukuri",
      "ngomba guhagarara munsi yitara ry’umutuku rimyatsa",
    ],
    correctAnswer: 3,
  },

  {
    id: 295,
    title: "wakora iki ubonye icyi cyapa ?",
    img: "../image/0.png",
    options: [
      "guhagarara gusa igihe ibinyabiziga bikwegereye",
      "guhagarara niyo  nta kinyabiziga ubona",
      "Guhagarara gusa niba hari abana bategereje kwambuka",
      "Guhagarara  gusa igihe  ikimenyetso cyaka  ari umutuku",
    ],
    correctAnswer: 1,
  },

  {
    id: 296,
    title: "Iki cyapa gisobanura iki?",
    img: "../image/247.png",
    options: [
      "Uguhinguka ku mwaro cyangwa ku nkombe cyangwa ahegereye icyome",
      "Inzira nyabagendwa iri kumusozi ucuramye",
      "Umuhanda utaringaniye",
      "Umuhanda wangijwe n’isuri",
    ],
    correctAnswer: 0,
  },

  {
    id: 297,
    title: "Iki cyapa gisobanura iki?",
    img: "../image/248.png",
    options: [
      "Hanyurwa na velomoteri gusa",
      "Nta modoka",
      "Hanyurwa nimodoka gusa",
      "Ntihanyurwa n’amapikipiki",
    ],
    correctAnswer: 3,
  },

  {
    id: 298,
    title: "Iki cyapa gisobanura iki?",
    img: "../image/250.png",
    options: [
      "Ahegereye amasangano y’inzira nyabagendwa n’inzira ya gari ya moshi ibambiye",
      "Inzira ibambiye imbere",
      "Inzira itabambiye itanafunze",
      "Imbere hari ikiraro  cy’amatungo",
    ],
    correctAnswer: 0,
  },

  {
    id: 299,
    title: "Iki cyapa gisobanura iki?",
    img: "../image/251.png",
    options: [
      "Umuhanda wubatswe nabi",
      "Agacuri  kateza ibyago",
      "Umuhanda utaringaniye",
      "Akazamuko gahanamye",
    ],
    correctAnswer: 1,
  },

  {
    id: 300,
    title: "Iki cyapa gisobanura iki?",
    img: "../image/252.png",
    options: [
      "Guhindura icyerekezo ibumoso ugana aho bahagarara",
      "Umuhanda udakomeza",
      "Nti byemewe guhindura icyerekezo  ibumoso",
      "Guhindura ikirekezo ibumoso  ugana ku cyome",
    ],
    correctAnswer: 1,
  },

  // start of test 2

  {
    id: 301,
    title: "Mu bimenyetso bimurika itara ritukura rivuga iki?",
    img: "../image/253.png",
    options: [
      "Hagarara kereste niba  ushaka gukata ibumoso",
      "Hagarara niba ubona ntabyago byaguteza",
      "Birabujijwe kurenga icyo kimenyetso",
      "Wemerewe kugenda niba aho asohokera mu masangano y’umuhanda hafunze",
    ],
    correctAnswer: 2,
  },

  {
    id: 302,
    title: "Mubimenyetso bimurika  itara ry’umuhondo risobanura iki?",
    img: "../image/254.png",
    options: [
      "Itegure kugenda",
      "Birabujijwe gutambuka umurongo wo guhagarara umwanya muto cg igihe uwo murongo udahari icyo kimenyetso ubwacyo",
      "A na b ni ibisubizo by’ukuri",
      "Nta gisubizo cy’ukuri kirimo",
    ],
    correctAnswer: 1,
  },

  {
    id: 303,
    title: "Mubimenyetso bimurika  itara ry’icyatsi risobanura iki?",
    img: "../image/255.png",
    options: [
      "Kwitegura kugenda",
      "Uburenganzira bwo kurenga icyo kimenyetso",
      "Hagarara niba inzira isohoka mu isangano ry’imihanda ifunze",
      "Ntagisubizo cyukuri kirimo",
    ],
    correctAnswer: 1,
  },

  {
    id: 304,
    title: "Umurongo ucagaguye wera mu muhanda usobanura iki?",
    img: "../image/256.png",
    options: [
      "Birabujijwe kuwurenga",
      "Birabujijwe kuhahagarara",
      "Wegereye ahaguteza ibyago",
      "Kunyuranaho ntibyemewe",
    ],
    correctAnswer: 0,
  },

  {
    id: 305,
    title: "Iki cyapa gisobanura iki?",
    img: "../image/257.png",
    options: [
      "Ukugendera mu muhanda ubisikanirwamo",
      "Ukugendera mu muhanda ubisikanirwamo ntibyemewe",
      "Cyerekana aho umunyegare agomba kunyura",
      "Nta gisubizo cy’ukuri  kirimo",
    ],
    correctAnswer: 0,
  },

  {
    id: 306,
    title: "Iki cyapa gisobanura iki?",
    img: "../image/258.png",
    options: [
      "Ahatangirwa serivisi ni muri metero 30",
      "Umuvuduko munini ntarengwa utegetswe ni 30 km/h",
      "Umuvuduko muto ntarengwa utegetswe ni 30 km/h",
      "Aho ibinyabiziga  bihagarara ni imbere mu birometero 30",
    ],
    correctAnswer: 2,
  },

  {
    id: 307,
    title: "Iki cyapa gisobanura iki?",
    img: "../image/259.png",
    options: [
      "Ahegereye umuhanda unyerera",
      "Imbere ipine ryapfumutse",
      "Ahegereye icyago kidasobanuye ukundi",
      "Imbere hari hatangirwa serivisi",
    ],
    correctAnswer: 2,
  },

  {
    id: 308,
    title: "Iki cyapa gisobanura iki?",
    img: "../image/260.png",
    options: [
      "Imbere hari umuyobozi w’amatungo",
      "Imbere hari inzira ya gari ya moshi",
      "Ahegereye amasangano y’inzira nyabagendwa n’inzira ya gari ya moshihatabambiye",
      "Inkomane ibambiye",
    ],
    correctAnswer: 2,
  },
  {
    id: 409,
    title: "Iki cyapa gisobanura iki?",
    img: "../image/248.png",
    options: [
      "Hanyurwa na velomoteri gusa",
      "Nta modoka",
      "Hanyurwa nimodoka gusa",
      "Ntihanyurwa n’amapikipiki",
    ],
    correctAnswer: 3,
  },

  {
    id: 412,
    title:
      "Mu gihe cy’impanuka mu muhanda n’ubundi bushotoranyi ni yihe nimero ya telefone y’ubutabazi wahamagara:",
    img: "../image/287.png",
    options: ["911", "100", "112", "131"],
    correctAnswer: 2,
  },

  {
    id: 309,
    title:
      " Muri iri sangano ry’umuhanda  hari icyapa gisobanura “guhagarara” n’umurongo wera urombereje munzira .Niyihe mpamvu hari iki cyapa cyo “guhagarara” hano?",
    img: "../image/261.png",
    options: [
      "Biragoye kubona neza mu muhanda munini",
      "Umuvuduko mu muhanda munini wavanyweho",
      "Ni mwisangano ry’umuhanda rikoreshwa cyane",
      "Hari imirongo iburira ibyago bitunguranye",
    ],
    correctAnswer: 0,
  },

  {
    id: 310,
    title:
      "Uhuye n’ingorane utwaye ikinyabiziga, mu muhanda ufite ibyerekezo bibiri, ufite ikimenyetso kiburira cya mpandeshatu. wagishyira mu ntera ingana iki uvuye aho ikinyabiziga cyahagaze",
    img: "../image/268.png",
    options: ["Metero 5", "Metero 25", "Metero 45", "Metero 100"],
    correctAnswer: 1,
  },

  {
    id: 311,
    title: "Umuyobozi usunika ipikipiki agomba gufatwa nka:",
    img: "../image/269.png",
    options: [
      "Umunyamaguru",
      "Umuyobozi w’ikinyabiziga",
      "Umugenzi",
      "A na b ni ibisubizo by’ukuri",
    ],
    correctAnswer: 1,
  },

  {
    id: 312,
    title: "Icyapa gikoze mw’ishusho ya mpandeshatu kimenyesha:",
    img: "../image/270.png",
    options: [
      "ibyago",
      "ibibujijwe",
      "ibitegetswe",
      "ntagisubizo cy’ukuri kirimo",
    ],
    correctAnswer: 0,
  },

  {
    id: 315,
    title: "Iki cyapa gisobanura:",
    img: "../image/271.png",
    options: [
      "uburenganzira bwo gutambuka mbere",
      "uburenganzira bwo gutambuka mbere mu yandi masangano  y’umuhanda akwegereye",
      "ibyago imbere mu masangano y’umuhanda ukwegereye",
      "a na b ni ibisubizo by’ukuri",
    ],
    correctAnswer: 0,
  },

  {
    id: 316,
    title: "Iki cyapa gisobanura:",
    img: "../image/272.png",
    options: [
      "Ntihanyurwa n’abanyamaguru",
      "Akayira kabanyamaguru",
      "Aho abanayamaguru bambukira",
      "B na c ni ibisubizo by’ukuri",
    ],
    correctAnswer: 0,
  },

  {
    id: 317,
    title:
      "Urenze munsisiro ,ukahasanga ibyapa bibiri iburyo bwawe  bimenyesha ko irangira ry’imirimo bitewe nicyo ibyo byapa bemenyesha  wagendera kuwuhe muvuduko?",
    img: "../image/273.png",
    options: ["70 km/h", "50 km/h", "40 km/h", "80 km/h"],
    correctAnswer: 3,
  },

  {
    id: 318,
    title: "Iki cyapa gisobanura iki?",
    img: "../image/274.png",
    options: [
      "Hoteli",
      "Ibitaro",
      "Ahagenewe kugwa kajugujugu",
      "B na c ni ibisubizo by’ukuri",
    ],
    correctAnswer: 1,
  },

  {
    id: 319,
    title:
      "igice kinzira nyabagendwa gikikijwe n’imirongo ibiri y’umweru iciyemo uduce kandi iteganye :",
    img: "../image/275.png",
    options: [
      "ahagenewe guhagarara umwanya munini n’umuto",
      "ahagenewe abanayamaguru",
      "ahagenewe inzira y’ibinyamitende",
      "a na b ni ibisubizo by’ukuri",
    ],
    correctAnswer: 2,
  },

  {
    id: 426,
    title: "Iki cyapa gisobanura iki?",
    img: "../image/417.png",
    options: [
      "Umuhanda utaringaniye i buryo",
      "Akamanuko gashobora gutera ibyago",
      "Ahantu umuhanda umeze nabi",
      "Uguhinguka ku mwaro cyangwa ku nkombe",
    ],
    correctAnswer: 1,
  },

  {
    id: 320,
    title: "iki cyapa kibuza abayobozi bibinyabiziga kunyuranaho :",
    img: "../image/237.png",
    options: [
      "iburyo",
      "ibumoso",
      "iburyo n’ibumoso",
      "nta gisubizo cy’ukuri kirimo",
    ],
    correctAnswer: 1,
  },

  {
    id: 321,
    title: "Iki  cyapa kibuza kunyuranaho ibumoso  ku binyabiziga bikurikira :",
    img: "../image/276.png",
    options: [
      "ku binyabiziga byose",
      "ku binyabiziga byose bifite moteri",
      "kubinyabiziga byose uretse ibinyamitende  ibiri n’amapikipiki adafite akanyabiziga ko k’uruhande",
      "nta gisubizo cy’ukuri kirimo",
    ],
    correctAnswer: 2,
  },

  {
    id: 322,
    title: "iki kimenyetso cyaka kinyemerera gukomeza:",
    img: "../image/278.png",
    options: [
      "yego",
      "yego ariko utanga inzira kubanyamaguru",
      "yego utanga inzira kubandi bayobozi b’ibinyabizaga baturutse mukindi cyerekezo",
      "oya",
    ],
    correctAnswer: 3,
  },

  // start of test3

  {
    id: 323,
    title:
      "Mu muhanda ufite uruhererekane rw’amakoni, feri y’urugendo ikoreshwa ryari?",
    img: "../image/279.png",
    options: [
      "Mbere ya buri koni",
      "Muri buri koni",
      "Nyuma ya buri koni",
      "Nta gisubizo cy’ukuri kirimo",
    ],
    correctAnswer: 0,
  },

  {
    id: 324,
    title: "Ni ikihe  cyapa muri ibi kintegeka gutanga inzira:",
    img: "../image/280.png",
    options: [
      "Icyapa A19",
      "Icyapa B5",
      "Icyapa B6",
      "Ntagisubizo cy’ukuri kirimo",
    ],
    correctAnswer: 1,
  },

  {
    id: 414,
    title: "Iki cyapa gisobanura Iki?",
    img: "../image/243.png",
    options: [
      "Iherezo Ry’umuhanda Wi Byerekezo Bibiri",
      "Iteme rinini Kandi rirerire",
      "Ifungana Ry’umuhanda",
      "Iherezo ry’iteme  rifunganye",
    ],
    correctAnswer: 2,
  },

  {
    id: 325,
    title: "Iki cyapa gisobanura:",
    img: "../image/281.png",
    options: [
      "utubuye dutaruka mu muhanda",
      "umuhanda urimo amazi",
      "umuhanda unyerera",
      "a na b ni bisubizo by’ukuri",
    ],
    correctAnswer: 0,
  },

  {
    id: 326,
    title:
      "Uri umuyobozi wa  velomoteri, uhuye n’umwe munshuti zawe agusaba ko wa mutwara ukamusiga ku’wundi muhanda. ufite imyaka 18 ariko nta  ngofero  yindi yabigenewe ufite. wamutwara?",
    img: "../image/282.png",
    options: [
      "yego",
      "yego usibye urugendo rurerure",
      "yego usibye urugendo rugufi",
      "Oya",
    ],
    correctAnswer: 3,
  },

  {
    id: 327,
    title:
      "Ni ikihe cyapa cy’inyemerera gutambuka mbere mu masangano y’umuhanda?",
    img: "../image/283.png",
    options: [
      "Icyapa B3",
      "Icyapa A22 a",
      "Icyapa A 20",
      "Ibisubizo byose ni ukuri",
    ],
    correctAnswer: 0,
  },

  {
    id: 328,
    title:
      "Mbere yo kunyura kumuyobozi w’ikinyabiziga cy’imitende ibiri, ngomba gucana akaranga cyerekezo k’ibumoso?",
    img: "../image/284.png",
    options: [
      "Yego buri gihe",
      "Yego igihe hari  ikinyabiziga kinkurikiye",
      "Yego iyo nkurikiwe nibindi binyabiziga by’imitende ibiri",
      "Oya nta na rimwe kunyura kubinyabiziga by’imitende ibiri",
    ],
    correctAnswer: 0,
  },

  {
    id: 329,
    title:
      "Nshobora kunyuraho umuyobozi w’ikinyabiziga wahagaze imbere y’inzira yabanyamaguru?",
    img: "../image/285.png",
    options: [
      "yego",
      "yego nyuma yo kuvuza ihoni",
      "yego mu gihe nkurikiwe n’ibindi binyabiziga",
      "Oya",
    ],
    correctAnswer: 3,
  },

  {
    id: 330,
    title: "Hejuru y’aka kanunga:",
    img: "../image/286.png",
    options: [
      "Nshobora kunyura ku kinyabiziga  icyo aricyose  mu gihe nagabanyije umuvuduko",
      "nshobora kunyura gusa kubinyabiziga by’imitende ibiri",
      "kunyuranaho ibumoso birabujijwe",
      "a na b ni ibisubizo by’ukuri",
    ],
    correctAnswer: 2,
  },

  {
    id: 331,
    title:
      "Mu gihe cy’impanuka mu muhanda n’ubundi bushotoranyi ni yihe nimero ya telefone y’ubutabazi wahamagara:",
    img: "../image/287.png",
    options: ["911", "100", "112", "131"],
    correctAnswer: 2,
  },

  {
    id: 332,
    title:
      "Ugeze bwa mbere ahabereye impanuka yo mu muhanda harimo inkomere wakora iki ? ",
    img: "../image/288.png",
    options: [
      "gusohora inkomere mu kinyabiziga",
      "kubaha icyo kunywa",
      "ku menyesha impanuka no guhamagara ubutabazi",
      "nta gisubizo cy’ukuri kirimo",
    ],
    correctAnswer: 2,
  },

  {
    id: 333,
    title:
      "N’iyihe myifatire myiza wagira ugeze aho abana bari hafi y’inzira nyabagendwa?",
    img: "../image/303.png",
    options: [
      "Itonde , witegereze ni biba ngongwa ubaburire unitegura kuba wahagarara",
      "Ihute urenge aho abo bana bari",
      "Komeza ugume ku muvuduko munini",
      "Komeza ugendere kuruhande rw’iburyo",
    ],
    correctAnswer: 0,
  },

  {
    id: 334,
    title:
      "Umuyobozi w’ikinyabiziga yegereye aho umwana w’umuhungu utwaye akagare k’abana asezera ku nshuti ye . N’iyihe myifatire myiza  wagira imbere yabo?",
    img: "../image/304.png",
    options: [
      "Ikomereze nkaho ataragera munzira nyabagendwa",
      "Itegure kureka uwo mwana w’umuhungu atambuke, kuko yajya mu muhanda atitaye ku kinyabiziga cyawe",
      "Gabanya umuvuduko ubwire uwo mwana yambuke ukoresheje ibimenyetso",
      "Komeza nkaho uwo mwana akiri munzira y’abanyamaguru",
    ],
    correctAnswer: 1,
  },

  {
    id: 335,
    title: "Nk’umuyobozi w’ikinyabiziga, n’iyihe myitwarire wagira?",
    img: "../image/305.png",
    options: [
      "Umuyobozi w’ikinyabiziga agomba gukomeza",
      "Umuyobozi w’ikinyabiziga agomba kuguma mu ruhande rw’iburyo kugira ngo ahe inzira umumotari",
      "Umuyobozi w’ikinyabiziga agomba gutegereza",
      "Umuyobozi w’ikinyabiziga agomba gutanga  inzira ayiha umu motari",
    ],
    correctAnswer: 0,
  },

  {
    id: 336,
    title:
      "Umuyobozi w’ikinyabiziga ageze hafi y’inzira y’abanyamaguru yakwitwara ate?",
    img: "../image/306.png",
    options: [
      "Kugabanya umuvuduko mu gihe cyiza, ukitegura guhagarara",
      "Gukomeza agendera ku muvuduko uri hejuru, mu gihe umunyamaguru ategereje",
      "Kuguma ku muvuduko yari afite mu gihe umunyamaguru atarambuka",
      "Kuvuza ihoni akaguma ku muvuduko yahozeho",
    ],
    correctAnswer: 0,
  },

  {
    id: 337,
    title:
      "Umuyobozi w’ikinyabiziga akurikiye ibinyabiziga bibiri, yifuza kubinyuraho. N’iki yashingiraho mbere yo kubanyuraho?",
    img: "../image/307.png",
    options: [
      "Ugomba kuzinyuraho zombi",
      "Sibyiza ko yazinyuraho atabasha kureba neza imbere ye",
      "Ibyapa by’aho ageze ntibimwemerera kunyuranaho",
      "Imbere har’inzira y’abanyamaguru",
    ],
    correctAnswer: 1,
  },

  {
    id: 338,
    title: "N’iki umuyobozi w’ikinyabiziga yakora ashaka gukatira iburyo?",
    img: "../image/308.png",
    options: [
      "Vuza ihoni umenyesha umunyegare ko ushaka gukatira iburyo",
      "Kata ikoni mbere y’umunyegare",
      "Emerera umunyegare akomeze inzira ye",
      "Ongera umuvuuko kugira ngo umutange gukata mbere ye",
    ],
    correctAnswer: 2,
  },

  {
    id: 339,
    title:
      "N’ayahe matara umuyobozi w’ikinyabiziga agomba gukoresha mugihe cy’ibihu?",
    img: "../image/309.png",
    options: [
      "Amatara kamena bihu y’imbere n’ay’inyuma hamwe n’amatara magufi",
      "Amatara kamenabihu y’imbere n’ay’inyuma",
      "Amatara magufi",
      "Urumuri rusanzwe",
    ],
    correctAnswer: 0,
  },

  {
    id: 340,
    title: "Muri ibi binyabiziga n’ikihe gihagaze nabi?",
    img: "../image/310.png",
    options: [
      "Ibinyabiziga byombi",
      "Ikinyabiziga cy’icyatsi",
      "Ikinyabiziga cy’umutuku",
      "Nta n’imwe",
    ],
    correctAnswer: 0,
  },

  {
    id: 341,
    title: "Ni gute umuyobozi w’ikinyabiziga yanyura kumunyegare hano?",
    img: "../image/311.png",
    options: [
      "Aha umuyobozi w’ikinyabiziga ntashobora kumunyuraho",
      "Atarenze umurongo wera ucagaguye",
      "Arenze umurongo wera ucagaguye",
      "Nta kurenga iyi mirongo yombi",
    ],
    correctAnswer: 2,
  },

  {
    id: 342,
    title:
      "N’iki umuyobozi w’ikinyabiziga yakora aramutse ahumishijwe n’urumuri rw’amatara yikinyabiziga giturutse mu kindi cyerekezo?",
    img: "../image/312.png",
    options: [
      "humisha ikinyabiziga giturutse mu kindi cyerekezo ucana  amatara maremare",
      "Egera kunkombe y’iburyo bw’umuhanda nibinashobioka ugabanye umuvuduko",
      "Canira amatara ikinyabiziga kiva mukindi cyerekezo",
      "Ongera umuvuduko kugira ngo usohoke mururwo rumuri vuba bishoboka",
    ],
    correctAnswer: 1,
  },

  {
    id: 411,
    title: "Iki cyapa gisobanura iki?",
    img: "../image/415.png",
    options: [
      "Inkomane banyuramo bazengurutse",
      "Biremewe guhindukira",
      "Inzira y’icyerekezo kimwe imbere",
      "Birabujijwe guhindukira",
    ],
    correctAnswer: 0,
  },

  // start of test4

  {
    id: 343,
    title:
      "Niki ugomba gukora igihe wegereye ikimenyetso kimurika kiva mucyatsi kijya mumuhondo?",
    img: "../image/313.png",
    options: [
      "Ongera umuvuduko kugirango usoze ikoni",
      "Komeza kuko itara ry’icyatsi rigiye kwaka",
      "Hagarara niba utateza ibyago",
      "Komeza ubwitonzi witegura guhagarara mugihe itara rihindutse umutuku",
    ],
    correctAnswer: 2,
  },

  {
    id: 428,
    title: "Iki kimenyetso kiri mu muhanda kivuze iki ?",
    img: "../image/431.png",
    options: [
      "Umuyobozi abujijwe kurenga umurongo wera ucagaguye  cyeretse mugihe bitateza icyago",
      "Birabujijwe kunyuranaho",
      "Biremewe kunyuranaho ariko nturenge umurongo wera ucagaguye",
      "Birabujijwe gusubira inyuma",
    ],
    correctAnswer: 0,
  },

  {
    id: 344,
    title:
      "Niki umuyobozi w’ ikinyabiziga akwiriye kumenya mugihe akurikiye umuyobozi wikinyamitende ibiri kandi imodoka y’ umweru iri gusubira inyuma ijya mumuhanda?",
    img: "../image/314.png",
    options: [
      "Umuyobozi wikinyabiziga gisubira inyuma azahagarara nabona umuyobozi w’ ikinyabimitende ibiri",
      "Umuyobozi w’ ikinyamitende ibiri ashobora gusaba umuyobozi w’ ikinyabiziga gisubira inyuma guhagarara",
      "Amatara yoguhagarara ashobora kuzima  ikinyabiziga gikomeza gusubira inyuma",
      "Umuyobozi w’ ikinyamitende ashobora guhagarara bitunguranye",
    ],
    correctAnswer: 3,
  },

  {
    id: 345,
    title:
      "Aha niki umuyobozi w’ ikinyabiziga yakora mugihe  ashaka kujya iburyo?",
    img: "../image/315.png",
    options: [
      "Gukomeza hagati y’ abanyamaguru babiri",
      "kuvuza ihoni akongera umuvuduko",
      "Guhagarara akareka abanyamaguru bakambuka",
      "Reka umunyamaguru umwe atambuke ubone umwanya wogutambuka",
    ],
    correctAnswer: 2,
  },

  {
    id: 346,
    title:
      "Aha umuyobozi w’ ikinyabiziga  ashobora kunyura kuri aba abanyamagare?",
    img: "../image/316.png",
    options: [
      "Oya, umuyobozi ntashobora kureba imbere neza",
      "Yego, kuko umurongo wera ucagaguye udashobora kuba udacagaguye",
      "Yego, abanyamagare bazumva imodoka bave munzira",
      "Yego, kuko buri kinyabiziga kiva mukindi cyerekezo gishobora kuguha inzira",
    ],
    correctAnswer: 0,
  },

  {
    id: 347,
    title: "Aha niki umuyobozi w’ ikinyabiziga yakora?",
    img: "../image/317.png",
    options: [
      "Kwemerera abanyamaguru kwambuka umuhanda",
      "Kuvuza ihoni agakomeza",
      "Tengereza munzira y’ abanyamaguru kugeza imidoka izimye",
      "Kongera umuvuduko mbere yuko abanyamaguru bambuka",
    ],
    correctAnswer: 0,
  },

  {
    id: 348,
    title:
      "Muri ibi binyabiziga bine ni ikihe kiri mu buryo bwiza bwo gukata ikoni ry’iburyo kiva mu muhanda munini kijya mu muto?",
    img: "../image/318.png",
    options: [
      "Ikinyabiziga cya mbere kiri mu buryo bwiza bwo gukata ikoni ry’iburyo",
      "Ikinyabiziga cya kabiri  kiri mu buryo bwiza bwo gukata ikoni ry’iburyo",
      "Ikinyabiziga cya gatatu kiri mu buryo bwiza bwo gukata ikoni ry’iburyo",
      "Ikinyabiziga cya kane kiri mu buryo bwiza bwo gukata ikoni ry’iburyo",
    ],
    correctAnswer: 0,
  },

  {
    id: 349,
    title: "Umurongo w’umweru urombereje uciye hagati mu muhanda uvuze iki?",
    img: "../image/324.png",
    options: [
      "Umuyobozi wese abujijwe kuwurenga",
      "Abanyamitende wemerewe kunyuranaho",
      "Kuhahagara biremewe",
      "Guhindukira ku manywa",
    ],
    correctAnswer: 0,
  },

  {
    id: 350,
    title:
      "Mbere yuko umuyobozi w’ikinyabiziga akata ibumoso mu  nzira nyabagendwa,  nihe ikinyabiziga kigomba kuba kiri ?",
    img: "../image/340.png",
    options: [
      "Mu ruhande rw’iburyo bw’inzira nyabagendwa",
      "Gusa iburyo bwo hagati y’inzira nyabagendwa",
      "Muruhande  urwarirwo rwo  hagati mu nzira nyabagendwa",
      "Mu ruhande rw’ibumoso bw’inzira nyabagendwa",
    ],
    correctAnswer: 1,
  },

  {
    id: 351,
    title:
      "Ugeze mu masangano y’umuhanda  aho usanga ibimenyetso bimurika bidakora, wakora iki igihe  umukozi ubifiye ububasha aguhaye iki kimenyesto ?",
    img: "../image/360.png",
    options: [
      "gukata ibumoso gusa",
      "gukata iburyo gusa ugakomeza imbere",
      "Guhagarara kumurongo wo guhagarara umwanya moto",
      "komeza imbere gusa",
    ],
    correctAnswer: 2,
  },

  {
    id: 352,
    title: "Iki cyapa kivuga:",
    img: "../image/243.png",
    options: [
      "Aho imihanda ihurira",
      "inkomane y’aho umuhanda umwe urasukira iburyo",
      "a na b ni ibisubizo by’ukuri",
      "nta gisubizo cy’ukuri kirimo",
    ],
    correctAnswer: 3,
  },

  {
    id: 353,
    title: "Iki cyapa gisobanura ibi bikurikira:",
    img: "../image/367.png",
    options: [
      "birabujijwe kunyura ku kindi kinyabiziga",
      "gutambuka mbere kw’ibinyabiziga bituruka aho ujya",
      "a na b ni ibisubizo by’ukuri",
      "nta gisubizo cyukuri kirimo",
    ],
    correctAnswer: 1,
  },

  {
    id: 354,
    title: "Iki cyapa kivuga:",
    img: "../image/369.png",
    options: [
      "iherezo ryo gutambuka mbere",
      "gutambuka mbere kw’ibinyabiziga biturutse imbere aho ujya",
      "gutambuka mbere y’ibinyabiziga biturutse imbere",
      "nta gisubizo cy’ukuri kirimo",
    ],
    correctAnswer: 2,
  },

  {
    id: 355,
    title: "Iki cyapa kigizwe:",
    img: "../image/370.png",
    options: [
      "ishusho mpandeshatu ,ubuso ubururu",
      "ishusho mpandeshatu,ubuso umukara",
      "ishusho mpandeshatu,ubuso umweru",
      "nta gisubizo cy’ukuri",
    ],
    correctAnswer: 2,
  },

  {
    id: 356,
    title: "Iki cyapa kivuga:",
    img: "../image/371.png",
    options: [
      "ifungana ry’umuhanda iburyo",
      "ifungana ry’umuhanda w’akayira gasatira umuhanda ibumoso",
      "akayira gato",
      "nta gisubizo cy’ukuri",
    ],
    correctAnswer: 1,
  },

  {
    id: 357,
    title: "Iki cyapa cyerekana:",
    img: "../image/373.png",
    options: [
      "ifungana ry’umuhanda",
      "ifungana ry’umuhanda n’akayira gasatira umuhanda i bumoso",
      "umuhanda utaringaniye",
      "nta gisubizo cy’ukuri kirimo",
    ],
    correctAnswer: 3,
  },

  {
    id: 358,
    title: "Icyi cyapa gisobanura :",
    img: "../image/374.png",
    options: [
      "ntihanyurwa mu byerekezo byombi",
      "ntihanyurwa n’abandi uretse abahatuye",
      "hanyurwa mu cyerekezo kimwe gusa",
      "nta gisubizo cy’ukuri kirimo",
    ],
    correctAnswer: 0,
  },

  {
    id: 359,
    title: "Icyi cyapa cyerekana: ",
    img: "../image/375.png",
    options: [
      "Ifungana ry’umuhanda",
      "umuhanda unyerera",
      "umuhanda utaringaniye",
      "nta gisubizo cy’ukuri kirimo",
    ],
    correctAnswer: 3,
  },

  {
    id: 360,
    title: "Iki cyapa kivuga:",
    img: "../image/376.png",
    options: [
      "ikoni iburyo",
      "akazamuko gashinze cyane",
      "akamanuko gashobora gutera ibyago",
      "b na c byose ni ukuri",
    ],
    correctAnswer: 0,
  },

  {
    id: 361,
    title:
      "Iki kimenyetso gitanzwe n’umukozi ubifitiye ububasha cyo guhagarara :",
    img: "../image/387.png",
    options: [
      "ku bakoresha umuhanda ba muturutse imbere",
      "ku bakoresha umuhanda bose bamuturutse imbere n’inyuma",
      "kubakoresha umuhanda bose bamuturutse inyuma",
      "nta gisubizo cy’ukuri kirimo",
    ],
    correctAnswer: 1,
  },

  {
    id: 362,
    title: "Ibi byapa byo mu muhanda birambuza kunyuranaho ibumoso ?",
    img: "../image/388.png",
    options: [
      "yego",
      "yego, iyo ufite umuvuduo wa 90km/h",
      "oya",
      "ntagisubizo cy’ukuri",
    ],
    correctAnswer: 0,
  },

  // start of test 5

  {
    id: 363,
    title:
      "Umuhanda urombereje w’ibice byinshi. Ndashaka kunyura kuri izi kamyo ibyiri  mugihe gito ibumoso icyarimwe , biremewe ?",
    img: "../image/390.png",
    options: ["yego", "oya", "yego bikorewe ibumoso", "ntagisubizo cy’ukuri"],
    correctAnswer: 2,
  },

  {
    id: 364,
    title: "K’umuyobozi w’ivatiri, iki cyapa kivuze iki ?",
    img: "../image/391.png",
    options: [
      "kirambuza gutwara ku muvuduko utarengeje 5km/h",
      "ntaburenganzira kimpa, mugihe gikurikizwa ku binyabiziga bifite hejuru y'atoni 5",
      "ntacyo bindebaho  mugihe bireba gusa zipima tone 5 no kurengaho",
      "Ntagisubizo cy’ukuri kirimo",
    ],
    correctAnswer: 2,
  },

  {
    id: 365,
    title: "Ndashaka gukata iburyo. Biremewe ?",
    img: "../image/392.png",
    options: [
      "yego",
      "yego, ariko nyuma yo guhagarara",
      "ntabwo byemewe",
      "ntagisubizo cyukuri kirimo",
    ],
    correctAnswer: 2,
  },

  {
    id: 366,
    title: "Umuhanda wambukiranya inzira ya gariyamoshi",
    img: "../image/393.png",
    options: [
      "nshobora gukomeza nkambuka umuhanda kubera ko uruzitiro rufunguye",
      "ngomba guhagarara munsi yitara ry’umutuku rimyatsa",
      "ntabwo nakomeza urugendo rwanjye. Ngomba gihita mpagarara",
      "ntagisubizo cy’ukuri",
    ],
    correctAnswer: 1,
  },

  {
    id: 367,
    title: "Kuri iki cyapa  cyo mu muhanda cyambere  kintegeka ?",
    img: "../image/394.png",
    options: [
      "Kugendera k’umuvuduko  uri hejuru ya 30km/h",
      "kutarenza umuvuduko wa 30km/h",
      "birabujijwe kugendera kumuvuduko uri hejuru ya 30km/h",
      "nta gisubizo cyukuri",
    ],
    correctAnswer: 2,
  },

  {
    id: 368,
    title: "Mpagaze mu murongo  wo guhagarara umwanya muto",
    img: "../image/395.png",
    options: [
      "Nshobora gukata iburyo",
      "Nshobora gukata ibumoso",
      "Nshobora gukata ibumoso  cyangwa iburyo ",
      "Ntagisubizo cy’ukuri kirimo",
    ],
    correctAnswer: 1,
  },
  {
    id: 369,
    title: "Ngomba:",
    img: "../image/396.png",
    options: [
      "guhagarara igihe gito kuri icyi cyapa cy’umuhanda",
      "guhagarara ngatanga inzira kuri metero 100 ntaragera kuri icyi cyapa",
      "gutanga inzira nkanahagarara  iyo ari ngombwa muri m100 ntaragera kuri icyi cyapa",
      "ntagisubizo cy’ukuri",
    ],
    correctAnswer: 1,
  },

  {
    id: 370,
    title:
      "Ndashaka kugata ibumoso. Imodoka y’icyatsi yaje irahagarara. Ninde ufite uburenganzira bwo gutambuka mbere?",
    img: "../image/397.png",
    options: [
      "mfite uburenganzira bwo gutambuka mbere",
      "imodoka y’icyatsi ifite uburenganzira bwo gutambuka mbere",
      "twembi ntaburenganzira bwo gutambuka mbere gusa tugomba gutambukana ubwitonzi",
      "ntagisubizo nakimwe kirimo",
    ],
    correctAnswer: 1,
  },

  {
    id: 371,
    title: "Mfite uburenganzira bwo gutambuka muri iri sangano ?",
    img: "../image/398.png",
    options: [
      "yego, niba ukata ibumoso",
      "Oya niba ukata iburyo",
      "yego , bitewe noho ngana",
      "ntagisubizo cy’ukuri kirimo",
    ],
    correctAnswer: 2,
  },

  {
    id: 372,
    title:
      "Ndi kumuvuduko wa 20km/h. nshobora gukomeza muri iri sangano ry’umuhanda?",
    img: "../image/399.png",
    options: [
      "oya",
      "yago, nshobora gukata iburyo",
      "yego, nshobora guta ibumoso cyangwa iburyo",
      "yego, nshobora gukata ibumoso gusa ",
    ],
    correctAnswer: 0,
  },

  {
    id: 373,
    title: "Umuyobozi wikinyabiziga aritegura kunyuraho ibumoso:",
    img: "../image/400.png",
    options: [
      "nshobora kumunyuraho nyuze iburyo",
      "sinshobora kumunyura",
      "nshobora kumunyura nciye ibumoso  ariko mbonye ko mfite umwanya uhagije",
      "Ntagisubizo cy’ukuri kirimo",
    ],
    correctAnswer: 2,
  },

  {
    id: 374,
    title: "Uhereye kuri ibi byapa habujijwe:",
    img: "../image/401.png",
    options: [
      "Kunyuranaho kubinyabiziga bikururwa nibinyabiziga birengeje imitende ibiri ibumoso no kugendera kumuvuduko urengeje 70 km/h",
      "Kunyuranaho kubinyabiziga bikururwa cyangwa ibinyabiziga birengeje imitende ibiri ibumoso",
      "kugendera hejuru ya 70 km/h",
      "ntagisubizo cy’ukuri",
    ],
    correctAnswer: 0,
  },

  {
    id: 375,
    title: "Ndashaka gupariki ikinyabiga iburyo kunzira y’abanyamaguru",
    img: "../image/402.png",
    options: [
      "biremewe munsi yicyi cyapa",
      "biremewe imbere y’icyi cyapa",
      "birabujijwe imbere n’inyuma yicyi cyapa",
      "nta gisubizo cy’ukuri kirimo",
    ],
    correctAnswer: 0,
  },

  {
    id: 376,
    title: "Iki cyapa gisobanura iki ?",
    img: "../image/403.png",
    options: [
      "Guhagarara, aho abanyeshuri bambukira",
      "Hagarara akanya gato",
      "Ibindi binyabiziga bigomba kuguha inzira",
      "Gutanga umwanya ku bindi binyabiziga i buryo bwawe",
    ],
    correctAnswer: 1,
  },

  {
    id: 377,
    title: "Iki cyapa gisobanura iki mu nkomane ?",
    img: "../image/404.png",
    options: [
      "Tanga inzira ku binyabiziga binini",
      "Gabanya umuvuduko uhe inzira abanyamaguru",
      "Tanga inzira ku binyabiziga bigenda mu muhanda munini wegera",
      "Tanga inzira ku ibinyabiziga biturutse iburyo bwawe",
    ],
    correctAnswer: 2,
  },

  {
    id: 378,
    title: "Iki cyapa gisobanura iki aho banyura bazengurutse ?",
    img: "../image/404.png",
    options: [
      "Tanga inzira ku binyabiziga biri mu muhanda munini",
      "Tanga inzira ku binyabiziga biturutse i bumoso",
      "Tanga inzira ku ma kamyo na za otobisi",
      "Ibinyabiziga byose uretse amapikipiki bigomba gutanga inzira",
    ],
    correctAnswer: 1,
  },

  {
    id: 379,
    title: "Iki cyapa gisobanura iki ?",
    img: "../image/406.png",
    options: [
      " Komeza imbere gusa",
      "Aho kunyuranaho imbere",
      "Aho guhagarara umwanya munini",
      "Inzira y’ icyerekezo kimwe",
    ],
    correctAnswer: 0,
  },

  {
    id: 380,
    title: "Iki cyapa gisobanura iki?",
    img: "../image/407.png",
    options: [
      "Umuhanda urombereje  w’ibice byinshi ibumoso",
      "Umuhanda uyoborejwe i bumoso",
      "Ibinyabiziga biturutse iburyo bifite uburenganzira bwo gutambuka mbere",
      "Kata i bumoso gusa",
    ],
    correctAnswer: 3,
  },

  {
    id: 381,
    title: "Iki cyapa gisobanura iki?",
    img: "../image/408.png",
    options: [
      "Kunyuranaho bikorerwa i buryo gusa",
      "Umuhanda uyoborejwe i buryo",
      "Kata i buryo gusa",
      "Umuhanda munini urasukira i bumoso",
    ],
    correctAnswer: 2,
  },

  {
    id: 382,
    title: "Iki cyapa gisobanura iki?",
    img: "../image/409.png",
    options: [
      "Birabujijwe gukata i buryo",
      "Tanga inzira ku bindi binyabiziga bigenda mu gihe ugiye gukatira iburyo",
      "Kata i buryo mu gihe nta bindi binyabiziga biturutse mu kindi cyerekezo",
      "Nta nkengero y’umuhanda yegutse iri i buryo",
    ],
    correctAnswer: 0,
  },

  {
    id: 423,
    title: "Imbere yawe  iki cyapa kikubwiye iki?",
    img: "../image/238.png",
    options: [
      "Umuvuduko ntarengwa wemewe",
      "Iherezo ry’ibyo wabuzwaga",
      "Guhagarara umwanya munini n’umwanya moto ntibyemewe",
      "Birabujijwe kuhinjira",
    ],
    correctAnswer: 2,
  },

  // start of test 6

  {
    id: 383,
    title: "Iki cyapa gisobanura iki?",
    img: "../image/410.png",
    options: [
      "Birabujijwe guhindukira",
      "Birabijijwe gusubira inyuma",
      "Umuhanda unyerera imbere",
      "Ntibyemewe kugendera mu byerekezo byombi",
    ],
    correctAnswer: 0,
  },

  {
    id: 384,
    title: "Iki cyapa gisobanura iki?",
    img: "../image/411.png",
    options: [
      "Umuhanda urombereje w’ibice byinshi ku birometero 50",
      "Intera nto ntarengwa ya metero 50 hagati y’ibinyabiziga",
      "Umuvuduko urenga ibirometero 50 mu isaha",
      "Umuvuduko ntarengwa ugarukira ku birometero 50 mu isaha",
    ],
    correctAnswer: 3,
  },

  {
    id: 385,
    title: "Iki cyapa gisobanura iki ?",
    img: "../image/412.png",
    options: [
      "Birabujijwe  ku binyabiziga bitwara abakozi ba leta",
      "Birabujijwe guhagara umwanya munini",
      "Birabujijwe ku binyabiziga by’abikorera ki giti cyabo",
      "Parikingi",
    ],
    correctAnswer: 1,
  },

  {
    id: 417,
    title: "Icyapa gikurikira  kivuze iki?",
    img: "../image/230.png",
    options: [
      "Ntihanyurwa",
      "Birabujijwe guhagarara umwanya munini",
      "Umuvuduko utarengeje",
      "Inzira yabanyeshuli",
    ],
    correctAnswer: 1,
  },

  {
    id: 386,
    title: "Iki cyapa gisobanura iki?",
    img: "../image/413.png",
    options: [
      "Aho guhagararwamo  n’abanyamagare imbere",
      "Aho abana bagenewe kwiga gutwara amagare",
      "Inzira y’iminyamitende n’abanyamaguru itegetswe",
      "Abanyamagare bagomba kuva ku igare  bakagendesha amaguru",
    ],
    correctAnswer: 2,
  },

  {
    id: 387,
    title: "Iki cyapa gisobanura iki ?",
    img: "../image/414.png",
    options: [
      "Uburemere ntarengwa bwemewe bwa toni 3",
      "Ntihanyurwa n’ibinyabiziga bigenewe gutwara ibicuruzwa",
      "Ntihanyurwa n’ibinyabiziga bifite imitambiko itatu",
      "Hanyurwa n’ibinyabiziga  bifite imitambiko itatu gusa",
    ],
    correctAnswer: 1,
  },

  {
    id: 388,
    title: "Iki cyapa gisobanura iki?",
    img: "../image/415.png",
    options: [
      "Inkomane banyuramo bazengurutse",
      "Biremewe guhindukira",
      "Inzira y’icyerekezo kimwe imbere",
      "Birabujijwe guhindukira",
    ],
    correctAnswer: 0,
  },

  {
    id: 410,
    title: "Icyapa gikoze mw’ishusho ya mpandeshatu kimenyesha:",
    img: "../image/270.png",
    options: [
      "ibyago",
      "ibibujijwe",
      "ibitegetswe",
      "ntagisubizo cy’ukuri kirimo",
    ],
    correctAnswer: 0,
  },

  {
    id: 389,
    title: "Iki cyapa gisobanura iki?",
    img: "../image/257.png",
    options: [
      "Umuhanda ugabanijwemo ibisate bibiri",
      "Umuhanda ugabanijwemo ibisate bine",
      "Inzira y’icyerekezo kimwe",
      "Ukugendera mu muhanda ubisikanirwamo",
    ],
    correctAnswer: 3,
  },

  {
    id: 425,
    title: "Mubimenyetso bimurika  itara ry’icyatsi risobanura iki?",
    img: "../image/255.png",
    options: [
      "Kwitegura kugenda",
      "Uburenganzira bwo kurenga icyo kimenyetso",
      "Hagarara niba inzira isohoka mu isangano ry’imihanda ifunze",
      "Ntagisubizo cyukuri kirimo",
    ],
    correctAnswer: 1,
  },

  {
    id: 390,
    title: "Iki cyapa gisobanura iki?",
    img: "../image/417.png",
    options: [
      "Umuhanda utaringaniye i buryo",
      "Akamanuko gashobora gutera ibyago",
      "Ahantu umuhanda umeze nabi",
      "Uguhinguka ku mwaro cyangwa ku nkombe",
    ],
    correctAnswer: 1,
  },

  {
    id: 391,
    title: "Iki cyapa gisobanura iki?",
    img: "../image/418.png",
    options: [
      "Akazamuko gashinze cyane",
      "Umuhanda utaringaniye i bumoso",
      "Ahantu umuhanda umeze nabi",
      "Ahegereye utununga",
    ],
    correctAnswer: 0,
  },

  {
    id: 392,
    title: "Iki cyapa gisobanura iki?",
    img: "../image/419.png",
    options: [
      "Akazamuko gashinze cyane",
      "Umuyaga w’intambike",
      "Uruhererekane rw’amakoni",
      "Umuhanda unyerera",
    ],
    correctAnswer: 3,
  },

  {
    id: 393,
    title: "Iki cyapa gisobanura iki?",
    img: "../image/420.png",
    options: [
      "Abana",
      "Inzira y’abanyamaguru – Itegure guhagarara",
      "Ikibuga cy’imikino cy’abana",
      "Ikibuga cy’ imyidagaduro",
    ],
    correctAnswer: 0,
  },

  {
    id: 419,
    title: "Iki cyapa gisobanura Iki?",
    img: "../image/243.png",
    options: [
      "Iherezo Ry’umuhanda Wi Byerekezo Bibiri",
      "Iteme rinini Kandi rirerire",
      "Ifungana Ry’umuhanda",
      "Iherezo ry’iteme  rifunganye",
    ],
    correctAnswer: 2,
  },

  {
    id: 394,
    title: "Iki cyapa gisobanura iki?",
    img: "../image/421.png",
    options: [
      "Isoko ry’amatungo",
      "Ivuriro ry’amatungo",
      "Uruzitiro rw’amatungo",
      "Akayira k’amatungo",
    ],
    correctAnswer: 3,
  },

  {
    id: 395,
    title: "Mu byapa bikurikira , ni ikihe cyerekana  umuhanda  udakomeza:",
    img: "../image/422.png",
    options: ["Icyapa  C1", "Icyapa E14", "Icyapa C2a", "Icyapa B2a"],
    correctAnswer: 1,
  },

  {
    id: 396,
    title: "Iki cyapa gisobanura iki?",
    img: "../image/404.png",
    options: ["Tanga inzira", "Icyago", "Icyerekezo kimwe", "Ntihanyurwa"],
    correctAnswer: 0,
  },

  {
    id: 397,
    title:
      "Muri ibi byapa bikurikira ni ikihe cyerekana ko umuyobozi ukibonye yemerewe gutambuka mbere y'abaturutse aho agana mu nzira ifunganye:",
    img: "../image/424.png",
    options: ["Icyapa B6", "Icyapa A19", "Icyapa B3", "Icyapa A22a"],
    correctAnswer: 0,
  },

  {
    id: 398,
    title: "Iki cyapa:",
    img: "../image/239.png",
    options: [
      "Aho banyura bazengurutse",
      "Cyerekana umuhanda w'ibisate bitatu",
      "Cyerekana ahegereye inkomane",
      "Ntagisuzo cy’ukuri kirimo",
    ],
    correctAnswer: 0,
  },

  {
    id: 399,
    title: "Iki cyapa gisobanura iki?",
    img: "../image/426.png",
    options: [
      "Hanyurwa mu cyerekezo kimwe",
      "Ntihanyurwa",
      "Umuhanda udakomeza",
      "Ntagisuzo cy’ukuri kirimo",
    ],
    correctAnswer: 1,
  },

  {
    id: 400,
    title: "Ni ikihe icyapa gisobanura umuhanda w'icyerekezo kimwe:",
    img: "../image/427.png",
    options: ["Icyapa D1a", "Icyapa E13a", "Icyapa C19", "Icyapa C1"],
    correctAnswer: 1,
  },

  {
    id: 401,
    title: "Iki cyapa gisobanura iki?",
    img: "../image/243.png",
    options: [
      "Aho imihanda ihurira",
      "Inkomane y’aho  umuhanda umwe urasukira iburyo",
      "Umuhanda udakomeza",
      "Nta gisubizo kirimo",
    ],
    correctAnswer: 3,
  },

  {
    id: 421,
    title: "Iki cyapa gisobanura iki?",
    img: "../image/234.png",
    options: [
      "Iherezo ry’ibibuzwa byose mu karere  ku binyabiziga bigenda",
      "Ntihemerewe kuhahagarara",
      "Umuvuduko ntarengwa wemewe",
      "Nta gisubizo cy’ukuri kirimo",
    ],
    correctAnswer: 0,
  },

  {
    id: 402,
    title: "Iki cyapa gisobanura iki?",
    img: "../image/242.png",
    options: [
      "Iherezo ry’ umuhanda urombeje w’ibice byinshi ",
      "Birabujijwe kunyura mu mu muhanda w’ ikindi cyerekezo",
      "Birabujijwe kunyuranaho",
      "Birabujijwe guhagara ku iteme",
    ],
    correctAnswer: 0,
  },

  // start of test 7

  {
    id: 403,
    title: "Iki cyapa gisobanura iki?",
    img: "../image/8.png",
    options: [
      "Ntihasohokerwa i bumoso mu nzira banyuramo bazengurutse",
      "Umuhanda udakomeza ibumoso",
      "Nta nkengero y’umuhanda yegutse iri ibumoso",
      "Birabujijwe gukata ibumoso",
    ],
    correctAnswer: 3,
  },

  {
    id: 404,
    title: "Iki kimenyetso kiri mu muhanda kivuze iki ?",
    img: "../image/430.png",
    options: [
      "Biremewe kunyuranaho",
      "Umuyobozi abujijwe kukirenga",
      "Wegereye icyapa cyo  guhagarara umwanya muto",
      "Umuhanda ufunganye",
    ],
    correctAnswer: 1,
  },

  {
    id: 405,
    title: "Iki kimenyetso kiri mu muhanda kivuze iki ?",
    img: "../image/431.png",
    options: [
      "Umuyobozi abujijwe kurenga umurongo wera ucagaguye  cyeretse mugihe bitateza icyago",
      "Birabujijwe kunyuranaho",
      "Biremewe kunyuranaho ariko nturenge umurongo wera ucagaguye",
      "Birabujijwe gusubira inyuma",
    ],
    correctAnswer: 0,
  },

  {
    id: 406,
    title:
      "Mu gihe  utwaye ikinyabiziga uva kuri A ugana kuri B,  Iki kimenyetso kiri mu muhanda kivuze iki ?",
    img: "../image/432.png",
    options: [
      "Umuyobozi ashobora kurenga umurongo wera udacagaguye mugihe  cyo guhindukira gusa",
      "Umuyobozi w’ikinyabiziga abujijwe kunyuranaho, uretse gusa abayobozi b’ibinyamitende nibo bashobora kurenga umurongo wera udacagaguye",
      "Umuyobozi w’ikinyabiziga abujijwe kunyuranaho arenze umurongo wera udacagaguye",
      "Umuyobozi w’ikinyabiziga ashobora kunyuranaho mu gihe bitateza icyago",
    ],
    correctAnswer: 3,
  },

  {
    id: 407,
    title:
      "Mu gihe  utwaye ikinyabiziga uva kuri A ugana kuri B,  Iki kimenyetso kiri mu muhanda kivuze iki ?",
    img: "../image/433.png",
    options: [
      "Umuyobozi w’ikinyabiziga ashobora kunyuranaho arenze umurongo wera udacagaguye",
      "Umuyobozi w’ikinyabiziga abujijwe kunyuranaho arenze imirongo yera",
      "Umuyobozi w’ikinyabiziga yemerewe kunyuranaho",
      "Abayobozi b’ibinyamitende gusa bemerewe kunyuranaho barenze umurongo wera udacagaguye",
    ],
    correctAnswer: 1,
  },
  {
    id: 427,
    title:
      "igice kinzira nyabagendwa gikikijwe n’imirongo ibiri y’umweru iciyemo uduce kandi iteganye :",
    img: "../image/275.png",
    options: [
      "ahagenewe guhagarara umwanya munini n’umuto",
      "ahagenewe abanayamaguru",
      "ahagenewe inzira y’ibinyamitende",
      "a na b ni ibisubizo by’ukuri",
    ],
    correctAnswer: 2,
  },
];


